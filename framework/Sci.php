...
{   
    ...
    public static function activation()
    {
        ...
        if ( version_compare( PHP_VERSION, self::$requirements['PHP'], '<' ) ) {
            $flags[] = 'PHP';
            $errorText .= '<p><strong>MVC WP Framework</strong> plugin requires PHP version '.self::$requirements['PHP'].' or greater.</p>';
        }

        if (version_compare($wp_version,  self::$requirements['WP'], '<' )) { 
            $flags[] = 'WordPress';
            $errorText .= '<p><strong>MVC WP Framework</strong> plugin requires WordPress version '.self::$requirements['WP'].' or greater.</p>';
        }
