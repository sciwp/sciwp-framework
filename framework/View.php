...
class View
{
    ...
    public function __construct($file, $plugin = null, $module = null, PluginManager $pluginManager)
    {
		...
		if ($plugin instanceof Plugin) {
			...
			$this->plugin = $this->pluginManager->get($plugin);
		} else if ($this->pluginManager->getMain()) {
			$this->plugin = $this->pluginManager->getMain();
		} else {
			throw new Exception('You should specify a plugin when creating a view.');
		}
