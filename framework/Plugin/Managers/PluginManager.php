...
class PluginManager extends Manager
{
    ...
    public function register($plugin, $pluginId = false, $addon = false)
    {
        ...
        $services = $plugin->config->get('services');
        if ($services) {
            foreach ($services as $key => $service) {
                $instance = Sci::make($service, [$pluginId, $plugin]);
                if (method_exists($service, 'configure')) {
                    $instance->configure();
                }
                $plugin->services()->add($key, $instance);
            }
        }
    ...
    {
        if ($this->mainPluginKey) return $this->plugins[$this->mainPluginKey];
        return false;
    }
}